...
const EditContinent = React.forwardRef((props, ref) => {
	const { open, onClose, continent, fetchData } = props;
	...
	useEffect(() => {
		if (continent) {
			setFormValue({
				nameRu: continent.name.ru,
				nameEn: continent.name.en,
				long: continent.central.long,
				lat: continent.central.lat,
			});
			setId(continent._id);
			setIsReady(true);
		}
