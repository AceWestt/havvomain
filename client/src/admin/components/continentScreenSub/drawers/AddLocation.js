...
const AddContinent = React.forwardRef((props, ref) => {
	const { open, onClose, fetchData, cont_id } = props;
		...
		if (formRef.current.check()) {
			...
			try {
				const { data } = await axios.post(
					`/api/locations/locations/${cont_id}`,
					body,
					config
				);
				...
				if (error.response?.data.error) {
					...
					toaster.push(errorMessage('Не удалось добавить локацию!'), {
						placement: messagePlacement,
					});
				}
			}
	...
	return (
		<Drawer ref={ref} open={open} onClose={onClose} size="md">
			<Drawer.Header>
				<Drawer.Title>Добавить локацию</Drawer.Title>
			</Drawer.Header>
			<Drawer.Body>
				<FormWrapper
